...
*/
(function(a){a.fn.popModal=function(b){function g(){function c(a){var d=Math.max(v,w,x);return("bottomLeft"==a?0<w:"bottomRight"==a?0<v:0<x)?a:0<w&&w==d?"bottomLeft":0<v&&v==d?"bottomRight":0<x&&x==d?"bottomCenter":a}if(f.inline)var b=e.position().left,h=e.position().top;else b=e.offset().left,h=e.offset().top;var m=parseInt(e.css("marginLeft"));parseInt(e.css("paddingLeft"));var g=parseInt(e.css("marginTop")),r=e.outerHeight(),k=e.outerWidth(),l=d.outerWidth(),p=d.outerHeight(),n,q=e.offset().left,
y=a(window).width()-e.offset().left-k,A=e.offset().top,B=q-10-l,v=k+q-l,C=y-10-l,w=k+y-l,x=Math.min(k/2+y-l/2,k/2+q-l/2),q=A-p/2;/^bottom/.test(f.placement)?n=c(f.placement):/^left/.test(f.placement)?n=0<B?"leftCenter"==f.placement&&0<q?"leftCenter":"leftTop":c("bottomLeft"):/^right/.test(f.placement)&&(n=0<C?"rightCenter"==f.placement&&0<q?"rightCenter":"rightTop":c("bottomRight"));d.removeAttr("class").addClass("popModal animated "+n);switch(n){case "bottomLeft":d.css({top:h+g+r+10+"px",left:b+
m+"px"}).addClass("fadeInBottom");break;case "bottomRight":d.css({top:h+g+r+10+"px",left:b+m+k-l+"px"}).addClass("fadeInBottom");break;case "bottomCenter":d.css({top:h+g+r+10+"px",left:b+m+(k-l)/2+"px"}).addClass("fadeInBottom");break;case "leftTop":d.css({top:h+g+"px",left:b+m-l-10+"px"}).addClass("fadeInLeft");break;case "rightTop":d.css({top:h+g+"px",left:b+m+k+10+"px"}).addClass("fadeInRight");break;case "leftCenter":d.css({top:h+g+r/2-p/2+"px",left:b+m-l-10+"px"}).addClass("fadeInLeft");break;
case "rightCenter":d.css({top:h+g+r/2-p/2+"px",left:b+m+k+10+"px"}).addClass("fadeInRight")}}function h(){d=a(".popModal");d.length&&(c(),l||(l=d.css("animationDuration"),l=void 0!==l?1E3*l.replace("s",""):0),setTimeout(function(){a(".popModal_source").replaceWith(a(".popModal_content").children());d.remove();a("html.popModalOpen").off(".popModalEvent").removeClass("popModalOpen")},l))}function c(){var a=d.attr("class"),c=a.replace("fadeIn","fadeOut");d.removeClass(a).addClass(c)}var e=a(this),d,
k="",m="",r="",p,f,l,n,q={init:function(c){f=a.extend({html:"",placement:"bottomLeft",showCloseBut:!0,onDocumentClickClose:!0,onDocumentClickClosePrevent:"",overflowContent:!1,inline:!0,asMenu:!1,beforeLoadingContent:"Please, wait...",onOkBut:function(){return!0},onCancelBut:function(){},onLoad:function(){},onClose:function(){}},c);if(0!==a("body").find(".popModal").length&&a("body").find(".popModal").attr("data-popmodal_id")==e.attr("data-popmodal_id"))h();else{a("html.popModalOpen").off(".popModalEvent").removeClass("popModalOpen");
a(".popModal_source").replaceWith(a(".popModal_content").children());a(".popModal").remove();"fixed"==e.css("position")&&(k="position:fixed;");f.asMenu?p=" popModal_asMenu":(p="",f.showCloseBut&&(m=a('<button type="button" class="close">&times;</button>')),r=f.overflowContent?" popModal_contentOverflow":"");n=(new Date).getMilliseconds();e.attr("data-popmodal_id",n);c=a('<div class="popModal animated" style="'+k+'" data-popmodal_id="'+n+'"></div>');var b=a('<div class="popModal_content'+r+p+'"></div>');
c.append(m,b);a.isFunction(f.html)?(b.append(f.beforeLoadingContent),f.html(function(c){b.empty().append(c);d=a(".popModal");g()})):("object"==a.type(f.html)&&f.html.after(a('<div class="popModal_source"></div>')),b.append(f.html));500>=a(window).width()&&(f.inline=!0);f.inline?e.after(c):a("body").append(c);d=a(".popModal");c=d.find(".popModal_footer");0!=c.length&&d.find(".popModal_content").css({marginBottom:c.outerHeight()+15+"px"});a.isFunction(f.html)||a.type(f.html);if(f.onLoad&&a.isFunction(f.onLoad))f.onLoad();
...
(function(a){a.fn.notifyModal=function(b){function g(){var c=a(".notifyModal");c.removeClass("open");setTimeout(function(){c.remove();-1!=d.duration&&clearTimeout(notifDur);if(d.onClose&&a.isFunction(d.onClose))d.onClose()},k)}var h=a(this),c,e="",d,k,m={init:function(b){d=a.extend({duration:2500,placement:"center",type:"notify",overlay:!0,onClose:function(){}},b);d.overlay&&(e="overlay");a(".notifyModal").remove();b=a('<div class="notifyModal '+d.placement+" "+e+" "+d.type+'"></div>');var m=a('<div class="notifyModal_content"></div>'),
f=a('<button type="button" class="close">&times;</button>');h=void 0===h[0]?h.selector:h[0].innerHTML;m.append(f,h);b.append(m);a("body").append(b);c=a(".notifyModal");k||(k=c.css("transitionDuration"),k=void 0!==k?1E3*k.replace("s",""):0);c.addClass("open");c.click(function(){g()});-1!=d.duration&&(notifDur=setTimeout(g,d.duration))},hide:function(){g()}};a("html").keydown(function(a){27==a.keyCode&&g()});if(m[b])return m[b].apply(this,Array.prototype.slice.call(arguments,1));if("object"===typeof b||
!b)return m.init.apply(this,arguments)};a("* [data-notifymodal-bind]").bind("click",function(){var b=a(this).attr("data-notifymodal-bind"),g={};void 0!==a(this).attr("data-duration")&&(g.duration=parseInt(a(this).attr("data-duration")));void 0!==a(this).attr("data-placement")&&(g.placement=a(this).attr("data-placement"));void 0!==a(this).attr("data-ontop")&&(g.onTop=/^true$/i.test(a(this).attr("data-ontop")));a(b).notifyModal(g)})})(jQuery);
(function(a){a.fn.hintModal=function(b){a(".hintModal").off();var g={init:function(){function b(c,e){var d,h=c.attr("class");d=c.outerWidth();var m=e.outerWidth(),g=c.offset().left,p=a(window).width()-c.offset().left-d,f=d+g-m,l=d+p-m,n=Math.min(d/2+p-m/2,d/2+g-m/2);d=c.hasClass("bottomLeft")?"bottomLeft":c.hasClass("bottomCenter")?"bottomCenter":c.hasClass("bottomRight")?"bottomRight":"bottomLeft";void 0===c.data("placement")&&c.data("placement",d);c.removeAttr("class").addClass(h+" "+function(a){var c=
Math.max(f,l,n);return("bottomLeft"==a?0<l:"bottomRight"==a?0<f:0<n)?a:0<l&&l==c?"bottomLeft":0<f&&f==c?"bottomRight":0<n&&n==c?"bottomCenter":a}(c.data("placement")))}a(this).on("mouseenter",function(){var c=a(this).find("> .hintModal_container");1<c[0].textContent.length&&(c.addClass("animated fadeInBottom"),b(a(this),c))});a(this).on("mouseleave",function(){var c=a(this).find("> .hintModal_container");1<c[0].textContent.length&&c.removeClass("animated fadeInBottom")})}};if(g[b])return g[b].apply(this,
...
function(c){var b;e.onOkBut&&a.isFunction(e.onOkBut)&&(b=e.onOkBut(c));!1!==b&&g();a(this).off("click");h.trigger("okbut")});c.find('[data-dialogmodal-but="cancel"]').on("click",function(){if(e.onCancelBut&&a.isFunction(e.onCancelBut))e.onCancelBut();g();a(this).off("click");h.trigger("cancelbut")});c.find('[data-dialogmodal-but="prev"]').on("click",function(){c.find(".dialogPrev").click()});c.find('[data-dialogmodal-but="next"]').on("click",function(){c.find(".dialogNext").click()})}function p(){u.empty().append(h[f].innerHTML);
...
e.top=""==e.type?0:"calc("+e.top+" + 60px)";var f=0,l=h.length-1;b=a('<div class="dialogModal '+e.type+'" style="top:'+e.topOffset+'px"></div>');var n=a('<div class="dialogModal_container" style="top:'+e.top+'"></div>'),q=a('<div class="dialogModal_top animated"></div>'),t=a('<div class="dialogModal_header"></div>'),u=a('<div class="dialogModal_body animated"></div>'),z=a('<button type="button" class="close">&times;</button>');b.append(n);n.append(q,u);q.append(t);t.append(z);u.append(h[f].innerHTML);
...
a.isFunction(e.onClose))e.onClose();h.trigger("close")});c.addClass("open");setTimeout(function(){""!=e.type&&n.css({opacity:1});q.addClass("fadeInTopBig");u.addClass("fadeInTopBig")},d+100);k();c.find(".dialogPrev").on("click",function(){0<f&&(--f,f<l&&c.find(".dialogNext").removeClass("notactive"),0===f&&c.find(".dialogPrev").addClass("notactive"),p())});c.find(".dialogNext").on("click",function(){f<l&&(++f,0<f&&c.find(".dialogPrev").removeClass("notactive"),f==l&&c.find(".dialogNext").addClass("notactive"),
...
b.append(e.append(d));d=g;c=a(".titleModal_container");var r=b.css("marginLeft"),p=b.css("marginTop"),f=b.css("marginBottom"),l=b.outerHeight(),n=b.outerWidth(),q=c.css("marginTop"),t=c.outerWidth(),u=c.outerHeight();"fixed"==b.css("position")||"absolute"==b.css("position")?g=e=0:(e=b.position().left,g=b.position().top);switch(d){case "bottom":c.css({marginTop:parseInt(q)-parseInt(f)+"px",left:e+parseInt(r)+(n-t)/2+"px"}).addClass("fadeInBottom");break;case "top":c.css({top:g+parseInt(p)-u+"px",left:e+
parseInt(r)+(n-t)/2+"px"}).addClass("top fadeInTop");break;case "left":c.css({top:g+parseInt(p)+l/2-u/2+"px",left:e+parseInt(r)-t-10+"px"}).addClass("left fadeInLeft");break;case "right":c.css({top:g+parseInt(p)+l/2-u/2+"px",left:e+parseInt(r)+n+10+"px"}).addClass("right fadeInRight")}}});e.on("mouseleave",function(){b=a(this);var d;if(void 0!==b.attr("data-title")){d=b.attr("data-title");b.removeAttr("data-title");b.attr("title",d);d=c.attr("class");var e=d.replace("fadeIn","fadeOut");c.removeClass(d).addClass(e);
c.remove()}})}};if(g[b])return g[b].apply(this,Array.prototype.slice.call(arguments,1));if("object"===typeof b||!b)return g.init.apply(this,arguments)};a(".titleModal").titleModal();document.addEventListener("DOMNodeInserted",function(b){a(b.target).hasClass("titleModal")&&a(".titleModal").titleModal()},!1)})(jQuery);
...
a.isFunction(e.onCancelBut))e.onCancelBut();g();a(this).off("click")});c.on("click",function(c){a(c.target).parents().andSelf().is(k)||b()});a("html").on("keydown.confirmModalEvent",function(a){27==a.keyCode&&b()})})()},hide:function(){g()}};if(k[b])return k[b].apply(this,Array.prototype.slice.call(arguments,1));if("object"===typeof b||!b)return k.init.apply(this,arguments)};a("* [data-confirmmodal-bind]").bind("click",function(){var b=a(this).attr("data-confirmmodal-bind"),g={};void 0!==a(this).attr("data-topoffset")&&
